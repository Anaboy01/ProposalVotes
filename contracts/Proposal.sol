...
contract ProposalVote {
    ...
    function voteOnProposal(uint8 _index, address _voter) external {
        require(msg.sender != address(0), "Zero address is not allowed");
        require(_index < proposals.length, "Out of bound!");
        require(!hasVoted[_voter][_index], "You've voted already");
        ...
        currentProposal.voters.push(_voter);

        hasVoted[_voter][_index] = true;
